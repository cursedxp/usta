...
| 🎓 **Pedagogy layer** | Opening drill (recall), explain-mode (Feynman), hint ladder (fading), prediction protocol (it won't tell you the `cargo check` result — it makes you predict first), spaced repetition (recall questions carry due dates on a 1→3→7→16→35→90-day ladder — the opening drill asks only what's due; the welcome box shows "Reviews due today: N"). |
