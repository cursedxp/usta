...
- 2026-08-16: TUI tasarım sistemi uygulaması — onaylı Claude Design projesi koda; `src/tui/theme.rs` tek kaynak (semantik renk `Color::Indexed` + glif çiftleri: `●❯·✓⚠✗▸`, tüm modüller + `ui.rs` plain-ANSI + termimad skin oradan beslenir); amber uyarı (179, ham `Color::Yellow` öldü), mor oyun (141), turuncu disiplini (durağan ≤2, test-kilitli); notice katmanları `· / ⚠ / ✗` (metin aynen, yalnız ön-ek+stil); gauge ≥%70 amber eşiği; stats/topics kolon hizası + başlık-altı `─` çizgi; exam kartı model-çizimli (GOAL.md `## Mock Exams` format kuralı, kabuk parse etmez) + game glif notu (TEACHING.md `▸`); davranış/metin regresyonu yok (yalnız sunum); v0.18.0.
