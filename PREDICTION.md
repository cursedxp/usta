...
- You may also get a one-line `[build state: the last cargo check that ran was failing — …]` note instead of the full block. It means no new check ran this turn — the verdict is from the last check that DID run, and nothing has re-verified the project since. Treat it as ground truth for that last check: do not declare the current step complete, and do not invent a fresher compile result than the note states.
