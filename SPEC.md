...
- **Konu girişi (v0.5, sonra rafine):** TTY promptunda **ret yok** — kısa yaz ya da cümleyle anlat. Dolgu kelimeleri (ben/bir/ile/yapmak/istiyorum/the/to…) atılır, Türkçe karakter sadeleştirilir, ilk 3 içerik kelimesi tire ile slug olur: "ben rust ile bir todo yapmak istiyorum" → `rust-todo`; "temel Linux güvenliği" → `temel-linux-guvenligi`. Cümle yazılırsa seçilen slug bildirilir. Detay yine sohbette. `usta start <konu>` ve pipe davranışı değişmedi.
