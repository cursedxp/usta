...
**v0.26.1 (CPR race fix):** upstream ratatui issue #2640 is open and unresolved — inline-viewport operations (`Terminal::clear()`'s `insert_before` path, and `compute_inline_size` at init/resize) each issue a cursor-position (CPR) query on stdin, and when a `crossterm::EventStream` already holds the stdin reader lock the query times out fatally (`Error: The cursor position could not be read within a normal duration`). Fixed on our side without waiting on upstream: `src/tui/backend_wrap.rs`'s `TrackedBackend<W>` wraps `CrosstermBackend` and answers `get_cursor_position()` from cursor state it tracks itself, so no CPR query reaches stdin during the session. The one real CPR query now happens exactly once, in `term::setup()`, before any `EventStream` is constructed, purely to seed that tracked state. This eliminates the fatal "cursor position could not be read" crash class entirely; the upstream bug itself remains open. Design: `docs/superpowers/specs/2026-08-27-cpr-race-fix-design.md`.
