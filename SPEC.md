...
- **Konu girişi (v0.5, v0.9 rafine):** TTY promptunda 1-3 kelimelik kısa öbek — Türkçe karakter sadeleştirilip tire ile slug'lanır ("temel Linux güvenliği" → `temel-linux-guvenligi`); 3+ kelimelik cümle yeniden sorulur (3 deneme, sonra ilk 3 kelimeden slug). Detay yine sohbette anlatılır. `usta start <konu>` ve pipe davranışı değişmedi.
